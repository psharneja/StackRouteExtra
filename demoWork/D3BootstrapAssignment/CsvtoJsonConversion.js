    ...
    if(firstColumn[i] != "World" )
      { 
        if (firstColumn[i]!="European Union")
         {
           populationArray[i-1] = {country : jsonArray[i][countryIndex], value : jsonArray[i][populationIndex] };
         }
       }
  }//for loop end
  ...
  for (var i = 1; i < noOfRows; i++)
   {
    if(firstColumn[i] != "World" )
      { 
        if (firstColumn[i]!="European Union")
         {
            gdpArray[i-1] = { country : jsonArray[i][countryIndex], value : jsonArray[i][gdpIndex] };
         }
       }//for loop end
   }
  ...
  for (var i = 1; i < noOfRows; i++)
   {
      ...
      { 
        if (firstColumn[i]!="European Union")
         {
            purchasingPowerArray[i-1] = { country : jsonArray[i][countryIndex], value : jsonArray[i][parityIndex] };
...
function jsonPurchaseGrowth( jsonArray )
 {
  ...
  for (var i = 1; i < noOfRows; i++)
   {      
      purchasegrowthArray[i-1] = { country : jsonArray[i][countryIndex], GrowthFrom2010to2013 : jsonArray[i][parityIndex]-jsonArray[i][parity2010Index],PopulationFrom2010to2013 : jsonArray[i][populationIndex]-jsonArray[i][population2010Index]  };
     
       
   }//for loop end
   //creating Json file for Purchasing Power 2013
  fs.writeFile(process.cwd() + "/json/PopulationAndPurchaseGrowth.json", JSON.stringify(purchasegrowthArray,undefined, 2), function (err)
   {
    if (err) throw err;
    console.log('Population and Purchasing Growth JSON file has been successfully created');
  });
 }
...
var d=0;
 ...
 d=0;
  ...
  { 
   ...
   {      
      aggregatePopulationArray[d] = { continents : populationCountries[i], aggregatePopulationof2013 : populationCountries[i+1]};
     d++;
