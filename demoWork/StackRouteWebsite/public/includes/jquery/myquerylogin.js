...
$(function (){
	...
	// To remove session - for testing purpose only - remove after site development
	// $.session.clear();console.log("Session Cleared - myquerylogin L8");
 		...
 		if (currUser != undefined)
 		{
 			$.ajax({ // Ajax call to validate email id
				...
				success : function (data) {
					...
					$.session.set('loggedInUserType', data.userType);
					$.session.set('loggedInUserBatch', data.batch);
					$.session.set('loggedInUserSubCategory', data.subCategory);
					$.session.set('loggedInUserName', data.name);
					$.session.set('loggedInUserEmailId',data.email);
					$.session.set('loggedcompleteProfile',data.completeProfile);
					$.session.set('loggedInUserContact',data.contactNumber);
				},
				error : function () {
					$.session.clear();
	...
	function validateLogin() // Validation Function
	{
		...
		$.ajax({ // Ajax call to validate email id
			type: 'GET',
			url: '/user?email=' + userName + '&logkey=' + passCode + '&registered=true',
			success : function (data) {
				if(data.length >= 1)
				{
	...
	{
		$('#load-sign-up').removeClass('show').addClass('hide');
		$('#load-sign-up').find("input[type=text], input[type=password], input[type=email], input[type=number]").val("");
		$('#load-sign-up .error-message').removeClass('show').addClass('hide');
		$('#load-sign-up .success-message').removeClass('show').addClass('hide');
		$('.sign-up-uname .success-message').removeClass('show').addClass('hide');
		$('.sign-up-uname .error-message').removeClass('show').addClass('hide');
	}
	...
	$('.sign-up-mobile input').on('input', function (e) { // Validate Mobile
